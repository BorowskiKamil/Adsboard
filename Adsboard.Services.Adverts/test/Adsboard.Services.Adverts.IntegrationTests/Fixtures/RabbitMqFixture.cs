...
namespace Adsboard.Services.Adverts.IntegrationTests.Fixtures
{
    public class RabbitMqFixture : IDisposable
    {
        ...
        public RabbitMqFixture()
        {
            _client = RawRabbitFactory.CreateSingleton(new RawRabbitOptions()
            {
                ...
                {
                    ioc.AddSingleton<INamingConventions>(new RabbitMqNamingConventions("adverts"));
                },
                Plugins = p => p  
                    .UseAttributeRouting()
                    .UseRetryLater()
